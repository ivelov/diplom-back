...
export function density2d(data, options: any = {}) {
  ...
  let [bwX = adjust * nrd(data, x), bwY = adjust * nrd(data, y)] = number2(
    options.bandwidth,
  );
  ...
  const estimator = {
    ...
    grid: () =>
      result ||
      (result = dericheConv2d(configX, configY, grid, [binsX, binsY])),
    extent: () => [
      [x0, x1],
      [y0, y1],
    ],
    heatmap: (
      { color, clamp, canvas, maxColors } = {
        color: undefined,
        clamp: undefined,
        canvas: undefined,
        maxColors: undefined,
